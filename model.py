...
class CausalSelfAttention3(nn.Module):
	def __init__(self, idx: int):
		...
		if self.idx == 0:
			self.c_attn = nn.Linear(self.dim, 2 * self.dim, bias=False)
	...
	def forward(self, x):
		...
		if self.idx == 0:
			self.k, self.v = self.c_attn(x).split(self.dim, dim=2)

		k = self.k.view(B, self.k.size(1), self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
		v = self.v.view(B, self.k.size(1), self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
		q = q.view(B, T if train else 1, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
		if train:
			if self.flash:
				...
				att = (q @ self.k.transpose(-2, -1)) * self.scale
...
class CausalSelfAttention(nn.Module):
	def __init__(self, idx: int):
		...
		self.dim = config.dim
		config.head_size = self.dim // config.nheads
		self.c_attnq = nn.Linear(self.dim,self.dim, bias=False)
		self.c_attn = nn.Linear(self.dim, 2 * self.dim, bias=False)
		self.c_proj = nn.Linear(self.dim, self.dim, bias=False)
		self.attn_dropout = nn.Dropout(config.dropout)
		self.resid_dropout = nn.Dropout(config.dropout)
	...
	def forward(self, x):
		B, T, C = x.size()
		q = self.c_attnq(x)
		k, v  = self.c_attn(x).split(self.dim, dim=2)
...
class Transformer(nn.Module):
	...
	def forward(self, 
		...
		for i, block in enumerate(self.blocks):
			# if i > 0:
			# 	block.causal_self_attention.k = self.blocks[0].causal_self_attention.k
			# 	block.causal_self_attention.v = self.blocks[0].causal_self_attention.v
