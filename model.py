...
class CausalSelfAttention(nn.Module):
	def __init__(self, idx: int):
		...
		self.attn_dropout = nn.Dropout(self.dropout) if not self.flash else None
	...
	def forward(self,
		...
		_, k, v = self.c_attn(y).split(self.dim, dim=2)
		k = k.view(B, T, self.nheads, self.hsize).transpose(1, 2)
		v = v.view(B, T, self.nheads, self.hsize).transpose(1, 2)
...
class SecondSelfAttention(nn.Module):
	def __init__(self, idx):
		...
		self.c_attn_q = nn.Linear(self.dim, self.dim, bias=config.bias)
		self.c_proj = nn.Linear(self.dim, self.dim, bias=config.bias)
	...
	def forward(self,
		...
		B, T, C = x.size()
		train = config.mode in ('train', 'test')

		q = self.c_attn_q(x if train else x[:,-1:])
		q = q.view(B, T if train else 1, self.n_heads, self.hsize).transpose(1, 2)
...
class Block(nn.Module):
	def __init__(self,
		...
		if self.idx > 0 and self.idx < config.nlayers - 1:
	...
	def forward(self,
		...
		x = x + self.heads(self.ln1(x), k, v, freqs_cis=freqs_cis)
...
class Transformer(nn.Module):
	def __init__(self) -> NoReturn:
		...
		if self.pos_method == 'rope':
			self.register_buffer('freqs_cis', precompute_freqs_cis(self.dim // self.nheads, config.block_size * 2))
		else:
			self.freqs_cis = None
		...
		self.alpha = 1.0 if not config.deepnorm else math.pow(2.0 * self.nlayers, 0.25)
		self.blocks = nn.ModuleList([Block(idx, self.alpha) for idx in range(self.nlayers)])
	...
	def forward(self, 
		...
		k, v = None, None
		x = self.blocks[0](x, k, v, freqs_cis=freqs_cis)
		k, v = self.blocks[0].heads.k, self.blocks[0].heads.v
		self.blocks[0].heads.k = None
		self.blocks[0].heads.v = None

		for i, block in enumerate(self.blocks[1:]):
			i += 1
			x = block(x, k, v, freqs_cis=freqs_cis)

			if i == self.nlayers - 1:
				continue

			k = k.transpose(1, 2).contiguous().view(B, T, self.dim)
			v = v.transpose(1, 2).contiguous().view(B, T, self.dim)
			k = block.k_norm(k + F.leaky_relu(block.k_linear(k), negative_slope=block.k_slope.item()))
			v = block.v_norm(v + F.leaky_relu(block.v_linear(v), negative_slope=block.v_slope.item()))
			k = k.view(B, T, self.nheads, self.hsize).transpose(1, 2)
			v = v.view(B, T, self.nheads, self.hsize).transpose(1, 2)
